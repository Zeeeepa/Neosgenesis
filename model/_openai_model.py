    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            payload_contract (`Any | None`, default `None`):
                兼容旧接口的参数，保留但当前实现不会处理契约逻辑。
        ...
        if structured_model:
            ...
            if not self.stream:
                ...
                stream = self._parse_openai_stream_response(
                    start_datetime,
                    response,
                    structured_model,
                )
                async def _stream_with_payload() -> AsyncGenerator[ChatResponse, None]:
                    async for item in stream:
                        yield self._attach_contract_payload(
                            item,
                            payload_contract,
                        )

                return _stream_with_payload()
        else:
        ...
        return self._attach_contract_payload(
            parsed_response,
            payload_contract,
        )
